      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="mt-10 space-y-6">
        {Object.keys(defaultValues || {}).map((field) => (
          <FormField
            ...
            render={(
              { field: fieldData } // Перейменуй тут field на fieldData, щоб не було конфлікту імен
            ) => (
              <FormItem className="flex w-full flex-col gap-2.5">
                <FormLabel className="paragraph-medium text-dark400_light700">
                  {fieldData.name === "email"
                    ? "Email Address"
                    : fieldData.name.charAt(0).toUpperCase() + fieldData.name.slice(1)}
                </FormLabel>
                <FormControl>
                  <Input
                    required
        ...
        >
          {form.formState.isSubmitting ? <LoadingSpinner /> : buttonText}
        </Button>
